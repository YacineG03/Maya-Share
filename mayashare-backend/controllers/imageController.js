...
exports.uploadImage = async (req, res) => {
  try {
    ...
    const dicomWebUrl = orthancId ? `wadouri:http://localhost:3000/wado?requestType=WADO&instanceID=${orthancId}` : null;
      ...
      Trace.deleteByImageId(imageId, (err) => {
        ...
        if (image.nomFichier.toLowerCase().endsWith('.dcm') && image.metadonnees) {
          ...
          if (metadonnees && metadonnees.orthancId) {
            ...
            axios.delete(`http://172.20.10.5:8042/instances/${metadonnees.orthancId}`, {
              auth: {
                username: ORTHANC_USER,
                password: ORTHANC_PASS,
              },
