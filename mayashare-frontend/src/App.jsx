  ...
  const handleLoginSubmit = async (formData, navigate) => {
    ...
    try {
      ...
      if (response.data.token) {
        ...
        if (userRole === "unknown") {
          setError("Votre rôle n'est pas reconnu dans le système. Veuillez contacter l'administrateur.");
          setLoading(false);
        ...
        setError("Connexion réussie, mais aucun token reçu. Veuillez réessayer.");
        setLoading(false);
        setTimeout(() => {
          navigate("/login");
        }, 300);
    ...
    <Router>
      ...
      <Routes>
        <Route
          path="/login"
          element={
            <Login onSubmit={handleLoginSubmit} error={error} loading={loading} />
          }
        />
        ...
        <Route
          ...
          element={
            isAuthenticated ? (
              role === "patient" || role === "medecin" || role === "infirmier" ? (
        ...
        <Route
          ...
          element={
            isAuthenticated ? (
              role === "patient" || role === "medecin" || role === "infirmier" || role === "admin" ? (
