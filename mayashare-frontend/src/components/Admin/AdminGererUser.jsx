...
function AdminGererUser() {
  ...
  const renderDesktopView = () => {
    return (
      <TableContainer component={Paper} sx={{ boxShadow: colors.shadow, borderRadius: 2, border: `1px solid ${colors.divider}` }}>
        <Table>
          <TableHead>
            ...
            {loading && !refreshing ? (
              ...
              <TableRow>
                <TableCell colSpan={9} align="center" sx={{ py: 4 }}>
                  <Typography color={colors.textSecondary}>Aucun utilisateur trouvé.</Typography>
                </TableCell>
              </TableRow>
            ) : (
              filteredUsers.map((user) => (
                ...
                >
                  ...
                  <TableCell sx={{ color: colors.textSecondary }}>{user.prenom || '-'}</TableCell>
                  <TableCell sx={{ color: colors.textSecondary }}>{user.role || '-'}</TableCell>
                  <TableCell sx={{ color: colors.textSecondary }}>{user.email}</TableCell>
                  <TableCell sx={{ color: colors.textSecondary }}>{user.telephone || '-'}</TableCell>
                  <TableCell sx={{ color: colors.textSecondary }}>{user.sexe || '-'}</TableCell>
                  <TableCell sx={{ color: colors.textSecondary }}>
                    {user.dateNaissance
                      ? new Date(user.dateNaissance).toLocaleDateString('fr-FR')
                      : '-'}
                  </TableCell>
                  <TableCell sx={{ color: colors.textSecondary }}>{user.idHopital || '-'}</TableCell>
                  <TableCell>
      ...
      <motion.div variants={containerVariants} initial="hidden" animate="visible">
        <Box sx={{ display: 'flex', alignItems: 'center', mb: 4, gap: 2 }}>
          ...
          <Box>
            ...
            <Typography variant="body1" color={colors.textSecondary} sx={{ mt: 0.5 }}>
              {dataFetched ? `${filteredUsers.length} utilisateur${filteredUsers.length !== 1 ? 's' : ''} au total` : 'Appuyez sur Actualiser pour charger'}
            </Typography>
          </Box>
          ...
          >
            ...
            >
              ...
              <Typography
                ...
                sx={{ mb: 2 }}
              >
                Veuillez cliquer sur le bouton Actualiser pour charger les utilisateurs
              </Typography>
              <motion.div
        ...
        <Dialog
          ...
          PaperProps={{ sx: { borderRadius: 3, boxShadow: colors.shadow, maxWidth: 400, bgcolor: colors.modalBackground } }}
        >
          <motion.div variants={dialogVariants} initial="hidden" animate="visible">
            <DialogTitle sx={{ fontWeight: 600, color: colors.text, pb: 1 }}>Ajouter un utilisateur</DialogTitle>
            <DialogContent>
        ...
        >
          <motion.div variants={dialogVariants} initial="hidden" animate="visible">
            <DialogTitle sx={{ fontWeight: 600, color: colors.text, pb: 1 }}>Modifier un utilisateur</DialogTitle>
            <DialogContent>
              <DialogContentText sx={{ mb: 2, color: colors.textSecondary }}>
