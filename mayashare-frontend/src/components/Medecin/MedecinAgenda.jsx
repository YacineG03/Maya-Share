...
const MedecinAgenda = () => {
  ...
  const getStatusLabel = (status) => {
    switch (status) {
      ...
      default:
        return <Chip label={"Inconnu"} color={"default"} sx={{ fontWeight: 500 }} variant="outlined" />
    }
  }
  ...
  const getMonthData = () => {
    ...
    for (let day = 1; day <= daysInMonth; day++) {
      const events = filteredRendezVous.filter((rdv) => {
        ...
        return rdvDate.getDate() === day && rdvDate.getMonth() === month && rdvDate.getFullYear() === year
      })
      ...
      <Box sx={{ mb: 4 }}>
        <Box sx={{ display: "flex", justifyContent: "space-between", alignItems: "center", mb: 2 }}>
          <Typography variant={"h5"} sx={{ fontWeight: 700, color: colors.text }}>
            <CalendarTodayIcon sx={{ mr: 1, verticalAlign: "middle", color: colors.primary }} />
            Agenda
          </Typography>
        ...
        <Box sx={{ display: "flex", flexWrap: "wrap", gap: 2, mb: 3 }}>
          ...
          >
            ...
            <Typography variant="h6" sx={{ fontWeight: 600, color: colors.error }}>
              {countAppointmentsByStatus("décliné") + countAppointmentsByStatus("annulé")}
            </Typography>
          </Paper>
        </Box>
      </Box>
            ...
            week.map((day, dayIndex) => (
              <motion.div
                ...
                sx={{
                  ...
                  backgroundColor: isToday(day) ? colors.today : day ? "white" : "#f5f5f5",
                  borderRadius: 2,
                  ...
                  <>
                    ...
                    >
                      {day.events.map((event, eventIndex) => {
                        ...
                        const startTime = eventDate.toLocaleTimeString("fr-FR", {
                          hour: "2-digit",
                          minute: "2-digit",
                        })
                        let color = colors.warning
                        if (event.etat === "accepté") color = colors.success
                        if (event.etat === "décliné" || event.etat === "annulé") color = colors.error
                        return (
                          <Chip
                            key={eventIndex}
      ...
      >
        <DialogTitle
          sx={{
            fontWeight: 600,
            pt: 3,
            backgroundColor: actionType === "view" ? colors.background : "white",
            borderBottom: actionType === "view" ? `1px solid ${colors.divider}` : "none",
          }}
        >
          {actionType === "view" &&
        ...
        >
          {actionType === "view" && selectedRdv?.events.length > 0 ? (
            <Box sx={{ maxHeight: "60vh", overflow: "auto" }}>
              {selectedRdv.events.map((event, index) => (
                <Box
                  ...
                  sx={{
                    p: 3,
                    borderBottom: index < selectedRdv.events.length - 1 ? `1px solid ${colors.divider}` : "none",
                    "&:hover": {
                      backgroundColor: colors.hover,
                    },
                  }}
                >
                  ...
                  <Box sx={{ display: "flex", alignItems: "center", mb: 1 }}>
                    <AccessTimeIcon sx={{ fontSize: 18, color: colors.textSecondary, mr: 1 }} />
                    <Typography sx={{ color: colors.textSecondary }}>{formatTime(event.dateRendezVous)}</Typography>
                  </Box>
                  ...
                  {event.nomInfirmier && (
                    <Box sx={{ display: "flex", alignItems: "center", mb: 1 }}>
                      <PersonIcon sx={{ fontSize: 18, color: colors.textSecondary, mr: 1 }} />
            ...
            >
              <CalendarTodayIcon sx={{ fontSize: 48, color: colors.textSecondary, mb: 2, opacity: 0.5 }} />
              <DialogContentText sx={{ color: colors.textSecondary, textAlign: "center" }}>
            ...
            <>
              ...
              {(actionType === "accept" || actionType === "decline" || actionType === "delete") && (
                <TextField
                  ...
                  error={(actionType === "decline" || actionType === "delete") && commentaire.trim() === ""}
                />
              )}
            </>
          )}
            ...
            <>
              ...
              <motion.div variants={buttonVariants} whileHover={"hover"} whileTap={"tap"}>
                <Button
                  ...
                  variant={"contained"}
                  autoFocus
                  disabled={(actionType === "decline" || actionType === "delete") && commentaire.trim() === ""}
                  sx={{
                    textTransform: "none",
                    fontWeight: 500,
                    borderRadius: 2,
