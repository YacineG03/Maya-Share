      ...
      <Modal open={open} onClose={onClose} sx={{ backdropFilter: 'blur(5px)' }}>
        ...
        >
          <Box sx={modalStyle}>
            ...
            >
              ID: {consultation.idConsultation} | Patient: {dossier?.nom}{' '}
              {dossier?.prenom}
            </Typography>
...
const InfirmierGererDossier = () => {
  ...
  const fetchConsultationsForDossier = async (dossier) => {
    ...
    try {
      ...
      toast.success(
        `Consultations chargées pour ${dossier.nom} ${dossier.prenom}`
      );
    } catch (error) {
      toast.error(
        `Erreur lors du chargement des consultations : ${error.response?.data?.message || 'Erreur inconnue'}`
      );
    } finally {
      setLoadingConsultations((prev) => ({
        ...prev,
        [dossier.idDossier]: false,
      }));
    }
  };
  ...
  const handleUpdateDossier = async (e) => {
    ...
    if (
      ...
      toast.error(
        'Groupe sanguin invalide. Veuillez sélectionner une option valide.'
      );
      return;
    }

    // Validation de la longueur des champs textuels
    const maxLength = 1000;
    if (editDossier.antecedentsMedicaux.length > maxLength) {
      toast.error(
        'Les antécédents médicaux ne peuvent pas dépasser 1000 caractères.'
      );
      return;
    }
    ...
    if (editDossier.notesComplementaires.length > maxLength) {
      toast.error(
        'Les notes complémentaires ne peuvent pas dépasser 1000 caractères.'
      );
      return;
    }
    ...
    try {
      const updateData = {
        ...editDossier,
        // Supprimer groupeSanguin si vide pour éviter d'envoyer une chaîne vide au backend
  ...
  const handleUploadImage = async (e) => {
    ...
    try {
      ...
      const dossier = dossiers.find(
        (d) => d.idDossier === selectedDossier.idDossier
      );
      if (dossier) {
        fetchConsultationsForDossier(dossier);
      }
      ...
      setDossiers((prevDossiers) =>
        prevDossiers.map((dossier) => ({
          ...
          consultations: dossier.consultations.map((consultation) => ({
            ...consultation,
            images: consultation.images.filter(
  ...
  const filteredDossiers = dossiers.filter((dossier) => {
    ...
    const matchesStatus =
      filterStatus === 'tous' || dossier.etat === filterStatus;
        ...
        <Box sx={{ display: 'flex', alignItems: 'center', gap: 2 }}>
          ...
          <Box>
            ...
            <Typography variant='body1' sx={{ color: colors.textSecondary }}>
              ...
              {filteredDossiers.length !== 1 ? 's' : ''} patient
                                ...
                                >
                                  ...
                                  <Box>
                                    ...
                                    <Typography
                                      ...
                                      sx={{ fontWeight: 500 }}
                                    >
                                      {dossier.notesComplementaires || 'Aucune'}
                                    </Typography>
                                  </Box>
                                </Box>
                              </Paper>
                            ...
                            {loadingConsultations[dossier.idDossier] ? (
                              ...
                              >
                                <Typography
                                  ...
                                  color={colors.textSecondary}
                                >
                                  Cliquez sur "Charger les consultations" pour
      ...
      >
        ...
        >
          ...
          >
            ...
            >
              Patient: {selectedDossier?.nom} {selectedDossier?.prenom} | ID:{' '}
