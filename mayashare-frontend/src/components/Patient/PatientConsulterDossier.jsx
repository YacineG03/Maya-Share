...
const FileViewerModal = ({ open, onClose, selectedFile }) => {
  ...
  // Logique pour générer l'URL DICOM
...
const ConsultationDetailsModal = ({ open, onClose, consultation }) => {
  ...
  // Vérifier si l'ordonnance est une URL ou un texte brut
  const hasOrdonnanceUrl = consultation.ordonnanceUrl && consultation.ordonnanceUrl.trim() !== '';
  const ordonnanceContent = consultation.ordonnance || 'Aucune ordonnance disponible';
            ...
            <Box hidden={activeTab !== 2}>
              ...
              >
                {hasOrdonnanceUrl ? (
                  <>
                    <iframe
                      src={`${API_URL}${consultation.ordonnanceUrl}`}
                      title={`Ordonnance ${consultation.idConsultation}`}
                      style={{
                        width: '100%',
                      ...
                      onError={() => {
                        ...
                        toast.error('Impossible de charger l ordonnance.');
                      }}
                    />
                    <Box sx={{ mt: 2, textAlign: 'right' }}>
                      <Button
                        ...
                        href={`${API_URL}${consultation.ordonnanceUrl}`}
                        download={`ordonnance_${consultation.idConsultation}.pdf`}
                        sx={{
                          bgcolor: colors.primary,
                  ...
                  <>
                    ...
                    <Button
                      variant='contained'
                      startIcon={<DownloadIcon />}
                      href={`data:text/plain;charset=utf-8,${encodeURIComponent(ordonnanceContent)}`}
