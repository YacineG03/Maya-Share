...
const PatientConsulterSuivi = () => {
  ...
  const handleCancelRendezVous = async () => {
    try {
      await updateRendezVous(selectedRdvId, {
        etat: 'annulé',
        commentaire: 'Annulation demandée par le patient',
      });
      setRendezVous(
        rendezVous.map((rdv) =>
          rdv.idRendezVous === selectedRdvId ? { ...rdv, etat: 'annulé' } : rdv
        )
      );
        ...
        <Box sx={{ display: 'flex', alignItems: 'center', mb: 4, gap: 2 }}>
          ...
          <Box>
            ...
            <Typography variant='body1' color='text.secondary' sx={{ mt: 0.5 }}>
              ...
              {activeTab !== 'tous' ? `(${activeTab})` : 'au total'}
            </Typography>
          </Box>
        </Box>
