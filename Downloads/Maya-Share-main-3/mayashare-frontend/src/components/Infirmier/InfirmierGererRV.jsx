...
const InfirmierGererRV = () => {
  ...
  const fetchSharedAgendas = async () => {
    try {
      ...
      const errorMessage =
        ...
        'Erreur de chargement des agendas partagés';
      toast.error(errorMessage);
      setPartages([]);
    }
  };
  ...
  const toggleExpandRdv = (rdv) => {
    setExpandedRdv(
      expandedRdv?.idRendezVous === rdv?.idRendezVous ? null : rdv
    );
  };
  ...
  const filteredRendezVous = rendezVous.filter((rdv) => {
    const matchesSearch =
      searchTerm === '' ||
      (rdv?.nomPatient?.toLowerCase() || '').includes(
        searchTerm.toLowerCase()
      ) ||
      (rdv?.prenomPatient?.toLowerCase() || '').includes(
        searchTerm.toLowerCase()
      ) ||
      (rdv?.nomMedecin?.toLowerCase() || '').includes(
        searchTerm.toLowerCase()
      ) ||
              ...
              <Grid xs={12} sm={6} md={4} key={rdv?.idRendezVous}>
                ...
                >
                  ...
                  >
                    ...
                    <Collapse
                      in={expandedRdv?.idRendezVous === rdv?.idRendezVous}
                    >
                      <CardContent sx={{ pt: 0 }}>
                        <Divider sx={{ mb: 2 }} />
                            ...
                            <Box>
                              ...
                              >
                                <Button
                                  ...
                                  onClick={() =>
                                    handleViewSharedAgenda(partage.idPartage)
                                  }
                                  sx={{
                                    bgcolor: colors.info,
                                    '&:hover': {
          ...
          {selectedPartage && (
            <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
              <Typography variant='body2' color='text.secondary' gutterBottom>
                Période du partage : du {formatDate(selectedPartage.dateDebut)}{' '}
                au {formatDate(selectedPartage.dateFin)}
              </Typography>
              <Calendar
                value={[
                  new Date(selectedPartage.dateDebut),
                  new Date(selectedPartage.dateFin),
                ]}
