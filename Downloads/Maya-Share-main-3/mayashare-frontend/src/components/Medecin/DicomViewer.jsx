...
const DicomViewer = ({ dicomWebUrl }) => {
  ...
  const activateTool = (toolName) => {
    ...
    if (toolName === 'Reset') {
      if (element) {
        ...
        try {
          ...
          console.error(
            "Erreur lors de la réactivation de l'outil Wwwc:",
            error
          );
        }
      }
    ...
    try {
      ...
      console.error(
        `Erreur lors de l'activation de l'outil ${toolName}:`,
        error
      );
    }
  };
  ...
  const applyAutoWindow = () => {
    ...
    if (element) {
      try {
        ...
        console.error(
          "Erreur lors de l'application de l'auto fenêtrage:",
          error
        );
      }
    }
    ...
    try {
      ...
      cornerstone.loadImage(dicomWebUrl).then(
        ...
        (error) => {
          ...
          setError(
            "Erreur lors du chargement de l'image DICOM: " + error.message
          );
          setIsLoading(false);
        }
      );
      ...
      setError(
        "Erreur lors de l'initialisation de Cornerstone: " + error.message
      );
      setIsLoading(false);
    }
        ...
        if (element) {
          ...
          toolList.forEach((tool) => {
            try {
              ...
              console.warn(
                `Impossible de désactiver l'outil ${tool} lors du nettoyage:`,
                error
              );
            }
          });
  ...
  return (
    ...
    >
      ...
      >
        <p style={{ margin: 0 }}>
          <strong>Instructions:</strong> Sélectionnez un outil puis cliquez et
          faites glisser sur l'image pour l'utiliser. Pour le zoom, faites
          glisser vers le haut pour agrandir, vers le bas pour réduire.
        </p>
        <p style={{ margin: '4px 0 0 0' }}>
          <strong>Débogage:</strong> Si les outils ne fonctionnent pas, vérifiez
